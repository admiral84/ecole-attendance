...
export default function UsersClient({
  ...
  // ---- Declared BEFORE the effect that uses it ----
  ...
  useEffect(() => {
    ...
    loadUsers();
  }, [isAuthenticated, userRole, router, loadUsers]);

  const handleRoleChange = async (targetUserId, newRole) => {
    setUpdatingRole(targetUserId);

    try {
      const { success, error } = await updateUserRole(targetUserId, newRole);

      if (success) {
        toast.success(
          `تم تغيير دور المستخدم إلى ${getRoleLabel(newRole)} بنجاح`,
        );
        await loadUsers();
      } else {
  ...
  const handleApprovalChange = async (targetUserId, currentApproved) => {
    setUpdatingApproval(targetUserId);
    const newApprovedStatus = !currentApproved;

    try {
      const { success, error } = await updateUserApproval(
        targetUserId,
        newApprovedStatus,
      );
  ...
  const handleDeleteUser = async () => {
    ...
    const targetId = userToDelete.user_id;
    ...
    try {
      const { success, error } = await deleteUser(targetId);

      toast.dismiss(loadingToast);

      if (success) {
        toast.success(
          `تم حذف المستخدم "${userToDelete.nom} ${userToDelete.prenom}" بنجاح`,
        );
        await loadUsers();
      } else {
    ...
    <div className="container mx-auto px-4 py-8">
            ...
            <div className="mb-6">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mt-3">
                <p className="text-yellow-800 text-sm">
                  ⚠️ ملاحظة: سيتم حذف المستخدم نهائياً من النظام. جميع السجلات
                  المرتبطة سيتم تعيين معرفها إلى NULL.
                </p>
              </div>
                ...
                users.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      {user.user_id !== userId ? (
                        <select
                          ...
                          className={`px-3 py-1 rounded-lg text-sm font-medium border focus:outline-none focus:ring-2 focus:ring-blue-500 ${getRoleBadgeColor(
                            user.role,
                          )} ${
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <button
                        onClick={() =>
                          handleApprovalChange(user.user_id, user.approved)
                        }
                        disabled={
      ...
      <div className="mt-6 flex justify-center">
        <button
          onClick={loadUsers}
          disabled={loading}
          className="text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-2 disabled:opacity-50"
          type="button"
        >
          {loading ? (
            <>
