...
export async function getStudentsWithPresentFalse() {
  try {
    ...
    const { data: absences, error: absencesError } = await query.order(
      "date_deb",
      { ascending: false },
    );
  ...
  try {
    ...
    if (userData.role === "teacher") {
      ...
      if (!seances || seances.length === 0) {
        return {
          success: false,
          error: "غير مصرح به - أنت غير مسؤول عن هذا القسم",
          data: [],
        };
      }
    }
...
export async function getAbsentStudentsByDateRange(date, filterType = "both") {
  try {
    ...
    let query = supabase.from("absence").select(
      `
        id,
        id_eleve,
        id_classe,
    ...
    const { data: absences, error } = await query.order("date_deb", {
      ascending: false,
    });
...
export async function markStudentPresent(
  ...
  try {
    ...
    // Verify the teacher is responsible for this class
    const { data: seances, error: seanceError } = await supabase
      .from("seance")
      .select("id")
      .eq("user_id", userData.user_id)
    ...
    if (seanceError) {
      return { success: false, error: seanceError.message };
    }
    ...
    if (studentUpdateError) {
      // Student status update failed, but absence was recorded.
      // Return success anyway so the caller reflects the main operation.
    }
...
export async function markStudentAbsent(
  ...
  try {
    ...
    if (!student) {
      return {
        success: false,
        error: "هذا التلميذ ليس مسجلاً في هذا القسم",
      };
    }
    ...
    if (existingAbsence) {
      return {
        success: false,
        error: "هذا التلميذ مسجل غائب بالفعل",
      };
    }

    // Insert the absence record
    const { error: absenceError } = await supabase.from("absence").insert({
      id_eleve: studentId,
      id_classe: classId,
      date_deb: startDate,
      heure_deb: startTime,
    ...
    if (studentUpdateError) {
      // Absence was recorded; student flag update is secondary.
    }
...
export async function sendAbsenceNotification(
  ...
  try {
    ...
    try {
      ...
      // Notification insert failed silently — the caller should not be blocked.
    }
    ...
    return { success: false, error: "حدث خطأ غير متوقع", teacherName: "" };
  }
}
...
export async function getAttendanceSummaryByStudent(studentId) {
  try {
    ...
    if (monthError) {
      // Current month count is best-effort; continue with 0.
    }
    ...
    return { success: false, error: "حدث خطأ غير متوقع", data: null };
  }
}

export async function checkNotificationsExist(
  classId,
  studentIds,
  absenceDates,
) {
...
export async function deleteNotification(studentId) {
  try {
    ...
    if (userData.role !== "teacher") {
      return {
        success: false,
        error: "غير مصرح به - فقط الأساتذة يمكنهم حذف الإشعارات",
      };
    }

    // Verify that this student belongs to a class taught by this teacher
    ...
    if (!seanceData || seanceData.length === 0) {
      return {
        success: false,
        error: "غير مصرح به - أنت غير مسؤول عن هذا التلميذ",
      };
    }
