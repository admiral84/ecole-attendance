...
export async function getSanctions(filters = {}) {
  try {
    ...
    const studentIds = [
      ...new Set(sanctionsData.map((s) => s.id_eleve).filter((id) => id)),
    ];
    const classIds = [
      ...new Set(sanctionsData.map((s) => s.id_classe).filter((id) => id)),
    ];
    ...
    const formattedData = sanctionsData.map((sanction) => {
      const studentInfo = studentMap.get(sanction.id_eleve) || {
        name: "غير معروف",
        num: "-",
      };
  ...
  try {
    ...
    if (sanctionsByClass && sanctionsByClass.length > 0) {
      const uniqueClassIds = [
        ...new Set(sanctionsByClass.map((s) => s.id_classe)),
      ];
    ...
    if (monthlyError) {
      return { success: false, error: monthlyError.message, data: null };
    }
